...
module Scrooge
  ...
  router.build do
    ...
    get    %r{^/accounts/(?<account_id>\d+)$}, Container.show_account,   :show_account
    patch  %r{^/accounts/(?<account_id>\d+)$}, Container.update_account, :update_account
    post   %r{^/accounts$},                    Container.create_account, :create_account
    delete %r{^/accounts/(?<account_id>\d+)$}, Container.delete_account, :delete_account
